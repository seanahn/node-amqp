...
Connection.prototype.addAllListeners = function() {
  ...
  self.addListener('error', function backoff(e) {
    // SEAN propagate connection errors to exchanges that may be in publish at the moment
    for (var key in this.exchanges) {
      var ex = this.exchanges[key];
      if(ex) ex.emit('error', e);
    }
...
// SEAN adding channel
Connection.prototype.channel = function (name /* options, openCallback */) {
  ...
  var c = new Channel(this, channel);
