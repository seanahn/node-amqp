...
var Queue = module.exports = function Queue (connection, channel, name, options, callback) {
  ...
  // SEAN
  this.basicGetListeners = [];
  this.currentGetListener;
  ...
  this.on('rawMessage', function(message) {
    if (message.consumerTag && self.consumerTagListeners[message.consumerTag]) {
      ...
      // SEAN handle get callbacks - rawMessage is emitted when a new message header arrives
      this.currentGetListener = this.basicGetListeners.shift();
      this.currentGetListener(message);
    }
  });
...
//SEAN get impl
Queue.prototype.get = function (options, messageListener) {
  var self = this;
...
Queue.prototype._onMethod = function (channel, method, args) {
  ...
  switch (method) {
    ...
    // SEAN support basicGet
    case methods.basicGetEmpty:
        debug('basicGetEmpty', util.inspect(args, null));
        this.currentGetListener = this.basicGetListeners.shift();
        if(this.currentGetListener) this.currentGetListener(null);
        break;
